...
启动方式：
    cd /user/sunweiyue/lib/swy-dev/Cyber_Eyes
...
from backend.core.schemas.common import Message, Role, TextContent, AudioContent, ImageContent, VideoContent, ContentItem
from backend.core.schemas.chat import ChatRequest, ChatResponse
from backend.core.schemas.streaming import (
    StreamingRequest, StreamingChunk, StreamingResponse, StreamingConfig,
)
from backend.core.schemas.duplex import (
    DuplexConfig, DuplexGenerateResult, DuplexPrefillRequest,
)
from backend.session_recorder import DuplexSessionRecorder, TurnBasedSessionRecorder, generate_session_id
...
def _compose_cyber_eyes_system_prompt(base_prompt: str, assist_context: Optional[Dict[str, Any]]) -> str:
    prompt = (base_prompt or "You are a helpful assistant.").strip()
    if not isinstance(assist_context, dict):
        return prompt
    if assist_context.get("app_id") != "cyber_eyes":
        return prompt

    profile = assist_context.get("session_profile") or {}
    labels = assist_context.get("labels") or {}
    prompt_fragments = assist_context.get("prompt_fragments") or {}
    frontend_hints = assist_context.get("frontend_hints") or {}
    hazard_priorities = _as_clean_list(assist_context.get("hazard_priorities"))
    output_contract = _as_clean_list(assist_context.get("output_contract"))

    lines: List[str] = [
        prompt,
        "",
        "Structured assist context from the frontend. Treat these as session facts, not optional hints:",
    ]
    ...
    for key, label in field_order:
        value = labels.get(key) or profile.get(key)
        if value:
    ...
    lines.append(f"- Vision HD: {'enabled' if profile.get('vision_hd') else 'disabled'}")

    if hazard_priorities:
        lines.append(f"- Hazard priorities: {' | '.join(hazard_priorities)}")
    if output_contract:
        lines.append(f"- Output contract: {' | '.join(output_contract)}")

    hint_lines: List[str] = []
    if frontend_hints.get("allow_user_interrupt"):
        hint_lines.append("user may interrupt at any time")
    if frontend_hints.get("prefer_action_first"):
        hint_lines.append("action-first responses")
    if frontend_hints.get("silence_when_stable"):
        hint_lines.append("stay quiet when the scene is stable")
    if frontend_hints.get("quick_crossing_mode"):
        hint_lines.append("crossing mode, compress speech aggressively")
    if frontend_hints.get("read_text_mode"):
        hint_lines.append("clear immediate collision risks before reading text")
    if hint_lines:
        lines.append(f"- Frontend control hints: {' | '.join(hint_lines)}")

    fragment_lines: List[str] = []
    for key in ("mission", "mobility", "pace", "style", "initiative"):
        fragment = prompt_fragments.get(key)
        if isinstance(fragment, str) and fragment.strip():
            fragment_lines.append(fragment.strip())
    if fragment_lines:
        lines.append("")
        lines.append("Frontend distilled guidance fragments:")
        lines.extend(fragment_lines)

    lines.append("")
    lines.append("Priority order: immediate hazard > safe direction of travel > current goal > other details.")
    lines.append("If this structured context conflicts with generic chat behavior, follow the structured context.")
    return "\n".join(lines)
...
class DuplexWsMessage(BaseModel):
    ...
    assist_context: Optional[Dict[str, Any]] = None
    # audio_chunk 参数
...
class MiniCPMOWorker:
    ...
    def load_model(self) -> None:
        ...
        from backend.core.processors.unified import UnifiedProcessor
...
app = FastAPI(title="Cyber Eyes Worker", lifespan=lifespan)
...
def _convert_to_model_msgs(schema_messages: List[Message]) -> list:
    ...
    from backend.core.processors.base import MiniCPMOProcessorMixin
...
async def duplex_ws(ws: WebSocket):
    ...
    try:
        while True:
            ...
            if msg_type == "prepare":
                raw_system_prompt = msg.get("system_prompt", "You are a helpful assistant.")
                assist_context = msg.get("assist_context")
                system_prompt = _compose_cyber_eyes_system_prompt(raw_system_prompt, assist_context)
                ref_audio_path = msg.get("ref_audio_path")
                ref_audio_b64 = msg.get("ref_audio_base64")
                # TTS ref audio: ????????allback ??ref_audio_base64?????????
                tts_ref_audio_b64 = msg.get("tts_ref_audio_base64") or ref_audio_b64
                config_dict = msg.get("config")
                if isinstance(assist_context, dict):
                    logger.info(
                        "Duplex assist_context received: app=%s schema=%s",
                        assist_context.get("app_id"),
                        assist_context.get("schema_version"),
                    )
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Cyber Eyes Worker")
