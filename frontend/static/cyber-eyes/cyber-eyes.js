...
function bindBackendTargetSelector() {
    const select = byId('backendTargetSelect');
    if (!select) return;

    const targets = listBackendTargets();
    select.innerHTML = '';
    ...
    select.value = activeTarget.id;
    renderBackendTargetMeta(activeTarget);

    select.addEventListener('change', () => {
        const nextTarget = setActiveBackendTarget(select.value);
        const serviceBadge = byId('serviceStatus');
        const sessionState = computeSessionState();
        const followUp = ['导盲中', '准备中', '恢复中'].includes(sessionState)
            ? '当前会话保持原连接，结束后再开始才会完全切到新目标。'
            : '新的请求和下一次导盲会话将使用这个目标。';

        renderBackendTargetMeta(nextTarget);
        if (serviceBadge) {
            serviceBadge.textContent = '切换目标中...';
        }
        announcePolite(`已切换到${nextTarget.label}。${followUp}`);
    });
}
