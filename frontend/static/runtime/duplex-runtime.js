/**
 * omni-app.js 鈥?Omni Full-Duplex page entry (Layer 2 ES Module)
 *
...
class LiveMediaProvider extends MediaProvider {
    ...
    async startPreview() {
        ...
        if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
            throw new Error('娴忚鍣ㄤ笉鏀寔 getUserMedia锛岃浣跨敤 HTTPS 璁块棶');
        }
        this._aborted = false;
    ...
    async start() {
        if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
            throw new Error('娴忚鍣ㄤ笉鏀寔 getUserMedia銆傝浣跨敤 HTTPS 璁块棶銆?);
        }
        if (!this._videoStream) {
            await this._openVideoStream(this._useFront);
        }
    ...
    async flipCamera() {
        ...
        addSystemEntry(`Camera: ${wantFront ? '鍓嶇疆' : '鍚庣疆'}`);
    }
...
class FileMediaProvider extends MediaProvider {
    ...
    constructor(file, opts = {}) {
        ...
        // Mic pipeline (modes: mic, mixed 鈥?AudioWorklet graph)
    ...
    async start() {
        ...
        if (rawDuration > FILE_MAX_DURATION) {
            addSystemEntry(`Video truncated: ${rawDuration.toFixed(1)}s 鈫?${cappedDuration}s`);
        }
        ...
        // 6. Prepare video element (don't play yet 鈥?playback timing controlled by phases)
        ...
        if (this._audioMode === 'video') {
            ...
            // During padding, file audio = zero 鈫?mix = mic only; frames = first/last frame
            this._paddedFrames = [
    ...
    _connectMic() {
        ...
        // mixed mode: connect padded video audio source [silence 脳 padBefore] + video + [silence 脳 padAfter]
        ...
        addSystemEntry(`Mic connected 鈥?AudioWorklet graph (${this._audioMode})`);
    }
    ...
    _startMainMicPhase() {
        // Delay video playback by padBefore 鈥?graph plays silence during leading padding
    ...
    _handleMicChunk(mixedAudio) {
        // During padding: file audio = zero 鈫?mix = mic only; frame = first/last
        // During main:    file audio = real 鈫?mix = mic + file; frame from video
        if (this._chunkIdx >= this._grandTotal) {
            this._disconnectMic();
            this._videoEl.pause();
    ...
    async _decodeVideoAudio(cappedDuration) {
        try {
            ...
            if (this._audioMode === 'mixed' && isFinite(srcLUFS)) {
                ...
                addSystemEntry(`Video audio: ${srcLUFS.toFixed(1)} LUFS 鈫?target ${targetLUFS} LUFS (auto ${autoGainDb.toFixed(1)} dB)`);
            } else {
                const foTarget = -28;
                const autoDb = isFinite(srcLUFS) ? foTarget - srcLUFS : 0;
                videoNormGain = Math.pow(10, autoDb / 20);
                addSystemEntry(`Video audio: ${isFinite(srcLUFS) ? srcLUFS.toFixed(1) : '鈥?} LUFS 鈫?${foTarget} LUFS (gain ${autoDb.toFixed(1)} dB)`);
            }
    ...
    pause() {
        ...
        // Mic/Mixed: suspend AudioContext (freezes entire graph 鈥?mic, file buffer, capture)
    ...
    resume() {
        ...
        if (this._audioMode === 'video') {
            ...
            if (this._chunkIdx < this._mainStart) {
                // Still in leading padding 鈥?schedule delayed play for remaining pad
                ...
                // In main phase 鈥?resume playback immediately
                this._videoEl.play();
            }
        }
...
function setButtonStates(running) {
    ...
    if (!running) {
        ...
        start.textContent = '鈼?Live';
        start.classList.add('live');
        if (fsStart) { fsStart.textContent = '鈼?Live'; fsStart.classList.add('live'); }
...
function flipCamera() {
    ...
    if (cameraPreview && cameraPreview._previewing) {
        cameraPreview.flipCamera().then(() => addSystemEntry(`Preview: ${cameraPreview._useFront ? '鍓嶇疆' : '鍚庣疆'}`)).catch(err => addSystemEntry(`Preview switch failed: ${err.message}`));
    }
}
...
async function startSession() {
    ...
    session.onCleanup = () => {
        ...
        // Finalize video recording (async 鈥?stop() returns Promise for MediaRecorder)
    ...
    let systemPromptText = document.getElementById('systemPrompt').value;
    const duplexPrepareHook = window.__duplexPrepareHook;
    if (typeof duplexPrepareHook === 'function') {
        try {
            const override = duplexPrepareHook({
                systemPrompt: systemPromptText,
                preparePayload: { ...preparePayload, config: { ...preparePayload.config } },
            });
            if (override && typeof override === 'object') {
                if (typeof override.systemPrompt === 'string') {
                    systemPromptText = override.systemPrompt;
                }
                if (override.preparePayload && typeof override.preparePayload === 'object') {
                    Object.assign(preparePayload, override.preparePayload);
                }
            }
        } catch (err) {
            console.warn('[omni] duplex prepare hook failed:', err);
        }
    }
...
// File playback volume slider 鈥?real-time update of video element volume
