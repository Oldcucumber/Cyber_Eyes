...
def main() -> None:
    parser = argparse.ArgumentParser(description='Fetch MiniCPM-o weights with ModelScope priority.')
    ...
    if args.source == 'skip':
        raise SystemExit('[model] --source skip was requested, but the model directory is not ready yet.')

    if args.local_source:
        local_source = Path(args.local_source).resolve()
        if not is_ready(local_source):
            raise SystemExit(f'[model] local source is not a ready MiniCPM-o directory: {local_source}')
        ensure_clean_dir(dest)
        copy_tree(local_source, dest)
        print(f'[model] copied local model into: {dest}')
        return

    ensure_clean_dir(dest)

    if args.source in {'auto', 'modelscope'} and try_modelscope(args.modelscope_id, dest):
        return

    ensure_clean_dir(dest)
    if args.source in {'auto', 'huggingface'} and try_huggingface(args.hf_id, dest):
        return

    raise SystemExit(
        '[model] failed to prepare MiniCPM-o-4_5. '
        'Use ModelScope first when possible, then fall back to Hugging Face, or pass --local-source.'
    )
