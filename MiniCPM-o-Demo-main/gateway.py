...
async def cyber_eyes():
    """Cyber Eyes product page"""
    if not app_registry.is_enabled("cyber_eyes"):
        return RedirectResponse(url="/", status_code=302)
    page_path = os.path.join(static_dir, "cyber-eyes", "cyber-eyes.html")
    if os.path.exists(page_path):
        return FileResponse(page_path)
    return HTMLResponse("<h1>Cyber Eyes</h1><p>Cyber Eyes page not found</p>")
